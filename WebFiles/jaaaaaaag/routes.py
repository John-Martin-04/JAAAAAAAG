from jaaaaaaag import app,db
from flask import render_template,redirect,url_for,request,flash,get_flashed_messages
from .models import User,Product,Cart,ConfirmedOrders
from jaaaaaaag.forms import RegisterForm,LoginForm,addtocartForm,confirmorderform
...
def contact():
    if request.method == 'POST':
        # You can process the contact form data here if needed
        name = request.form.get('name')
        email = request.form.get('email')
        message = request.form.get('message')
        # You could store the message in a database, send an email, etc.
        flash(f"Thank you for contacting us, {name}!", category="success")
        return redirect(url_for('home'))
